...
function drawMonth(){
	...
	document.getElementById("prevMonth").innerHTML = "<img src='/images/buttons/arrowBlockLeft.png' class='arrowBlock' />";
	document.getElementById("prevMonth").title = "Back to "+myMonths[prevDate.getMonth()].toLowerCase()+" "+prevDate.getFullYear();
	prevDate.setYear(dateObj.getFullYear());
	prevDate.setMonth(dateObj.getMonth()+1);
	document.getElementById("nextMonth").innerHTML = "<img src='/images/buttons/arrowBlockRight.png' class='arrowBlock' />";
	document.getElementById("nextMonth").title = "Forward to "+myMonths[prevDate.getMonth()]+" "+prevDate.getFullYear();
	try{
...
function handleDroppedObject($ele,elid){
	...
	if ($('#'+reminderElId).is('.reminderPortlet'))
		...
		if (newReminderDesc == 'Birthday')
			...
			if (bd != null)
			    {
                newReminderDesc = bd.substring(0,60) + "'s "+newReminderDesc;
		...
		if (newReminderDesc == 'Custom')
			{
			var bd = prompt ("What is the reminder for ?");
			if (bd != null)
			    {
                newReminderDesc = bd.substring(0,60);
                newReminderPeriod = '365';
			    }
			else
			    {
...
function saveReminder(){
	...
	if (rmDesc == "")
	    {
	    document.getElementById("rmDesc").focus();
	    alert("You need to enter a description to save this reminder");
	    }
    else
        {
...
$(document).ready(function() {
    ...
    if ($.browser.msie)
        ...
        $( ".reminderPortlet" ).draggable({ 	revert: "valid" ,
                                                ...
                                                cursorAt: { cursor: "move", top:50, left: 10}
                                        });
        }
    else
        {
